...
float envelope_get(long long frame)
{
    ...
    if (start_frame > release_frame)
    {
        ...
        {
            // decay
            return (1.0 - ((1.0 - sustain_value) * (frame - decay_start)) / decay_frames);
        }
        // attack
        return (1.0 * (frame - start_frame)) / attack_frames;
    }
    else
    {
        // Release
        int diff = frame - release_frame;
        return max(0.0, sustain_value * (1.0 - diff / (release_frames)));
    }
}
